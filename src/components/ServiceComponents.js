  ...
  return (
    <div className="alternating-container">
      {sections.map((section, index) => {
        ...
        const directionClass = isEven ? 'left' : 'right';
        
        return (
          <div
            key={index}
            ref={el => sectionRefs.current[index] = el}
            className={`alternating-section ${directionClass} ${isVisible ? 'visible' : ''}`}
          >
            ...
            <div className="alternating-image">
              <div 
                ...
                style={{ background: section.color || 'linear-gradient(135deg, #2c3e50 0%, #1a1a2e 100%)' }}
              >
                {section.icon && <span className="large-icon">{section.icon}</span>}
              </div>
            </div>
          </div>
        );
...
// 3.5 Alternating Content Blocks with Images
